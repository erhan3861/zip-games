...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function init() {
        ...
        // Support per-level links via ?level=XYZ (overrides saved state)
        const urlParams = new URLSearchParams(window.location.search);
        const levelParam = parseInt(urlParams.get('level'), 10);
        const isValidLevelParam = !isNaN(levelParam) && levelParam >= 1;

        const restoredState = isValidLevelParam ? null : persistence.loadFullGameState();

        if (restoredState) {
            logic.startLevel(restoredState.level, restoredState);
        } else {
            const level = isValidLevelParam ? levelParam : persistence.loadLevel();
            const points = persistence.loadPoints();
            if (isValidLevelParam) {
