...
async function loadFixedLevelData(level, callback) {
    try {
...
export function startLevel(levelNumber, restoredState = null) {
    ...
    if (restoredState) {
        ...
        // Force unpaused on restore to avoid frozen screen
        isPaused = false;
    ...
    if (restoredState) {
        ...
        levelGenerator.generateLevelAsync(gridRows, gridCols, (hamiltonianPath) => {
            ...
            loadFixedLevelData(level, (levelData) => {
                // If level JSON specifies a different grid size, adopt it
                const jsonRows = Array.isArray(levelData?.grid) ? levelData.grid.length : null;
                const jsonCols = Array.isArray(levelData?.grid?.[0]) ? levelData.grid[0].length : null;
                if (jsonRows && jsonCols && (jsonRows !== getState().gridRows || jsonCols !== getState().gridCols)) {
                    const newCellSize = calculateCellSize(jsonRows, jsonCols, currentLevelParams.baseCellSize);
                    updateState({ gridRows: jsonRows, gridCols: jsonCols, calculatedCellSize: newCellSize });
                }

                const newWallPositions = new Set();
                const newWaypointPositions = new Set();
                const finalNumberPositions = {};

                // Convert fixed level data to game format
                levelData.walls.forEach(wall => {
                    const wallKey = `${wall.type}_${wall.row}_${wall.col}`;
                    newWallPositions.add(wallKey);
                });

                levelData.waypoints.forEach(waypoint => {
                    const waypointKey = `${waypoint.row}-${waypoint.col}`;
                    newWaypointPositions.add(waypointKey);
                });

                levelData.numbers.forEach(number => {
                    const numberKey = `${number.row}-${number.col}`;
                    finalNumberPositions[numberKey] = number.value;
                });

                // Ensure xCells matches the highest number in JSON
                const maxNumber = levelData.numbers.reduce((m, n) => Math.max(m, n.value), 1);
                if (maxNumber && maxNumber !== getState().xCells) {
                    updateState({ xCells: maxNumber });
                }

                // Optional: per-level success code from JSON (e.g., {"successCode":"ABC-123"})
                if (typeof levelData.successCode === 'string' && levelData.successCode.trim().length > 0) {
                    updateState({ levelSuccessCode: levelData.successCode.trim() });
                } else {
                    updateState({ levelSuccessCode: null });
                }
...
function handleWin() {
    // Mark game over without enabling next-level flow
    updateState({ isGameOver: true, isLevelCompletePendingNext: false });
    timer.stopTimer();
    ...
    // Do not preserve a "pending next" state
    persistence.saveFullGameState();

    ui.showMessage(`Level ${getState().level} Complete! Points: ${newPoints}`);
    if (getState().levelSuccessCode) {
        ui.showSuccessCode(getState().levelSuccessCode);
    }
    audio.playSound('soundWin');
    // Keep reset/restart available; disable active input
    ui.disableAllInput();
    ui.updateButtonStates(getState());
}
...
export function requestResetLevel() {
    ...
    if (level > 101 && points < config.RESET_PENALTY && !isLevelCompletePendingNext) {
...
export function performRestart() {
    try { persistence.clearFullGameState(); } catch (_) {}
    // Reload the current page and preserve any ?level= param
